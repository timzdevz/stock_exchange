...
if ( ! function_exists( 'woo_load_frontend_css' ) ) {
	function woo_load_frontend_css () {
		wp_register_style( 'theme-stylesheet', get_stylesheet_uri(), false, '2.0.15' );
...
/*		if (! is_page('stock') && ! bp_is_my_profile() && $_COOKIE['desktop_ver'] !== 1 ) {
		...
		if ( is_array($woo_options) ) {
			...
			if ( isset( $woo_options['woo_typography'] ) && $woo_options['woo_typography'] == 'true' ) {
				...
				if ( isset( $woo_options['woo_font_homepage_widget_titles'] ) && $woo_options['woo_font_homepage_widget_titles'] )
					$output .= '#wrapper #content.home-widgets .widget_woo_component h2.widget-title { '.woo_generate_font_css($woo_options[ 'woo_font_homepage_widget_titles' ]).' }'  . "\n";
