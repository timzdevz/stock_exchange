...
?>
	<div id="content" class="home-widgets">
		<?php $is_paged = is_paged(); ?>
		<?php woo_featured_slider_loader(); ?>
		<?php
			if ( is_home() && ! dynamic_sidebar( 'homepage' ) ) {

		    	if ( !$is_paged && 'true' == $settings['homepage_enable_features'] ) {
		    	...
		    	if ( !$is_paged && 'true' == $settings['homepage_enable_posts_timeline'] ) {
		    		get_template_part( 'includes/posts-timeline' );
		    	}

		    	if ( !$is_paged && is_woocommerce_activated() && 'true' == $settings['homepage_enable_hero_product'] ) {
		    		get_template_part( 'includes/hero-product' );
		    	}

		    	if ( !$is_paged && is_woocommerce_activated() && 'true' == $settings['homepage_enable_featured_products'] ) {
		    		get_template_part( 'includes/featured-products' );
		    	}

		    	if ( !$is_paged && 'true' == $settings['homepage_enable_testimonials'] ) {
